...
// Importação de outras classes (exceção personalizada para campos obrigatórios)
...
public abstract class Usuario {
    ...
    public Usuario(String nome, String cpf, String email,
        ...
        if (cargo == null || cargo.isBlank()) {
            throw new CampoObrigatorioException("Cargo é obrigatório");
        }
        if (login == null || login.isBlank()) {
            throw new CampoObrigatorioException("Login é obrigatório");
        }
    ...
    public void setId(int id) { // Sem validação no setter — é "uso interno" pelo DAO depois que o banco gera o
                                // id.
        this.id = id;
    }
    ...
    // Cada subclasse (Administrador, Gerente, Colaborador) é obrigada a
    // implementar.
